...
var (
	...
	hotkeysByID  = make(map[uint32]*Hotkey)
	nextHotkeyID uint32
)

func addHotkey(hk *Hotkey) uint32 {
	for {
		...
		hotkeysMu.Lock()
		if _, exists := hotkeysByID[id]; !exists {
			hotkeysByID[id] = hk
			hotkeysMu.Unlock()
			return id
		}
...
func removeHotkey(id uint32) {
	hotkeysMu.Lock()
	delete(hotkeysByID, id)
	hotkeysMu.Unlock()
}

func getHotkey(id uintptr) *Hotkey {
	hotkeysMu.RLock()
	hk := hotkeysByID[uint32(id)]
	hotkeysMu.RUnlock()
	return hk
}
...
func (hk *Hotkey) register() error {
	...
	id := addHotkey(hk)
	ret := C.registerHotKey(C.int(mod), C.int(hk.key), C.uint32_t(id), &hk.hkref)
	if ret != 0 {
		removeHotkey(id)
		return fmt.Errorf("failed to register the hotkey (macOS status %d)", int32(ret))
	}
...
func (hk *Hotkey) unregister() error {
	...
	if ret != 0 {
		return fmt.Errorf("failed to unregister the current hotkey (macOS status %d)", int32(ret))
	}
...
func keydownCallback(id uintptr) {
	hk := getHotkey(id)
	if hk == nil {
		return
	}
