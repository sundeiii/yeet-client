...
//		"github.com/sundeiii/yeet-client/internal/hotkey/mainthread"
//	)
//
...
//	}
//
// [mainthread]: see the mainthread package next to this one
...
func newEventChan() (chan<- Event, <-chan Event) {
	...
	go func() {
		...
		for {
			// If we don't have pending events, always read from input first.
			if len(q) == 0 && !closed {
				e, ok := <-in
				if !ok {
					close(out)
			...
			// Drain queued events to output. If input is closed while draining,
			// keep flushing the queue and close output without blocking forever.
			for len(q) > 0 {
				var nextIn chan Event
				if !closed {
					nextIn = in
				}
